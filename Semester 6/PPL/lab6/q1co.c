...
int main(void) {
    ...
    cl_kernel kernel = clCreateKernel(program, "strprint", &ret);
    ...
    ret = clSetKernelArg(kernel, 3, sizeof(int), &len);

    size_t global_item_size = len; // Process the entire string
    ...
    double total_time = (double)(time_end - time_start) / 1000000.0; // Convert nanoseconds to milliseconds

    char *strres = (char*)malloc(sizeof(char) * N * len);
